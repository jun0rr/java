...
public class TestObjectSpec {
  ...
  public void test_person_object_spec() {
    try {
      ObjectSpec<Person> spec = ObjectSpec.createSpec(Person.class, new BitBoxConfiguration());
  ...
  public void test_address_object_spec() {
    try {
      ObjectSpec<Address> spec = ObjectSpec.createSpec(Address.class, new BitBoxConfiguration());
      spec.getters().forEach(System.out::println);
  ...
  public void test_object_transform() {
    try {
      ...
      BitBoxConfiguration cfg = new BitBoxConfiguration();
      BitTransform<Person> trans = cfg.getTransform(Person.class);
