...
/**
 * BeanReader utility class to generate DcrCtu
 * JavaBean from a given java.sql.ResultSet.
 */
public class DcrCtuReader {

  private final ResultSet rset;

  /**
   * Default constructor receiving the ResultSet.
   * @param rs ResultSet containing the DcrCtu information.
   */
  public DcrCtuReader( ResultSet rs ) {
    if(rs == null) {
	...
	public static DcrCtuReader of(ResultSet rs) {
		return new DcrCtuReader(rs);
	}
  ...
  private java.sql.Date getDate(ResultSet rs, String col) {
    try {
  ...
  /**
   * Create a DcrCtu bean with the ResultSet information.
   * @return The created DcrCtu bean.
	 * @throws SQLException In case of error reading the ResultSet.
   */
  public IDcrCtu readBean() throws SQLException {
    IDcrCtu bean = new DcrCtu();
    ...
    if(this.contains( "dt_inc_ctu" )) {
      bean.setDtIncCtu( getDate(rset, "dt_inc_ctu") );
    }
    if(this.contains( "dt_fim_ctu" )) {
      bean.setDtFimCtu( getDate(rset, "dt_fim_ctu") );
    }
    if(this.contains( "dt_atl_ctu" )) {
      bean.setDtAtlCtu( getDate(rset, "dt_atl_ctu") );
    }
    ...
    if(this.contains( "dt_dtq" )) {
      bean.setDtDtq( getDate(rset, "dt_dtq") );
    }
    return bean;
  }
