...
public class SSOParser {
	 ...
	 * informações do usuário.
	 */
	public MappedUser parse() throws IOException {
		int index1 = response.indexOf("userdetails.role=id=");
		int index2 = response.indexOf("userdetails.attribute.name=");
		if(index1 < 0 || index2 < 0) {
			throw new IOException("Resposta invalida: nao contem detalhes do usuario. " + response);
		}
    ...
    if(!map.containsKey(SSOField.CHAVE.getName()) 
        || map.get(SSOField.CHAVE.getName()) == null 
        || map.get(SSOField.CHAVE.getName()).trim().isEmpty()) {
    }
