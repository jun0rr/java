 ...
 * @author Juno
 */
public class TestHttpConnectionReuse {
  ...
  private TcpChannel server() {
    ...
    ChannelHandlerSetup<HttpHandler> setup = HttpServerHandlerSetup.newSetup()
        .addResponseFilter(()->x->{
          ...
          x.channel().events().write(x).execute();
          return x.empty();
        })
        .addResponseFilter(()->x->{
  ...
  public void method() throws InterruptedException {
    try {
      ...
      HttpRequest req = HttpRequest.of(HttpVersion.HTTP_1_1, HttpMethod.POST, "/echo", "Hello World Netty 1");
      req.headers().set(HttpHeaderNames.ACCEPT, HttpHeaderValues.TEXT_PLAIN)
          .set(HttpHeaderNames.HOST, "https://localhost:4321");
      postecho.events().write(req).executeSync();
      Thread.sleep(5000);
      req = HttpRequest.of(HttpVersion.HTTP_1_1, HttpMethod.POST, "/echo", "Hello World Netty 2");
      req.headers().set(HttpHeaderNames.ACCEPT, HttpHeaderValues.TEXT_PLAIN)
          .set(HttpHeaderNames.HOST, "https://localhost:4321");
      postecho.events().write(req).executeSync();
      count.await();
      server.events().shutdown().onShutdown(g->System.out.println("[SERVER] Shutdown complete")).executeSync();
      postecho.events().shutdown().onShutdown(g->System.out.println("[POST /echo] Shutdown complete")).executeSync();
    }
    catch(Exception e) {
      e.printStackTrace();
      throw e;
