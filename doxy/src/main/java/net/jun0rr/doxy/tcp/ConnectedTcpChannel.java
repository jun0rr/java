 ...
 * @author juno
 */
public class ConnectedTcpChannel implements TcpChannel {
  ...
  public ConnectedTcpChannel shutdown() {
    future.get().addListener(f->((ChannelFuture)f).channel().close()
        .addListener(ff->future.set(group.shutdownGracefully())));
    return this;
  }
